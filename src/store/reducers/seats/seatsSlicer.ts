...
export const seatsSlicer = createSlice({
  ...
  reducers: {
    ...
    addCheckedSeat: (state, action: PayloadAction<ISeat>) => {
      const currentMovie = findMovie(state.bookedMovies, action.payload.id);
      if (currentMovie) {
        const currentReservedSeat = currentMovie.reserved.includes(
          action.payload.seatNumber
        );
        if (currentReservedSeat) {
          return;
        }
        const currentCheckedSeat = currentMovie.checked.includes(
          action.payload.seatNumber
        );
        if (currentCheckedSeat) {
          currentMovie.checked = currentMovie.checked.filter(
            (item) => item !== action.payload.seatNumber
          );
          currentMovie.totalCheckedPrice =
            currentMovie.totalCheckedPrice - action.payload.price;
...
export const { setCheckedMovie, addCheckedSeat, addReservedSeat } =
  seatsSlicer.actions;
